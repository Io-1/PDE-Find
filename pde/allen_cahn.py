...
"""
Allen–Cahn PDE system implementation with configurable library build.
"""
import numpy as np
# from pysindy import PDELibrary

# Default picklable feature functions/naming in library_functions.py

# Default parameters
D_DEFAULT: float = 0.01  # diffusion coefficient
R_DEFAULT: float = 1.0   # reaction strength

def simulate_allen_cahn(
    D: float = D_DEFAULT,
    r: float = R_DEFAULT,
    N: int = 128,
    L: float = 1.0,
    ...
    """
    1D Allen–Cahn equation (explicit reaction, implicit diffusion)
...
def get_target_coefs(
    ...
    """
    Build ground-truth coefficient matrix of shape (F,1) for the Allen–Cahn equation.
    """
    F = len(feature_names)
    coefs = np.zeros((F, 1))       # 2-D shape (features × 1 eq)

    mapping = {
        'uₓₓ':  D_param,         # u_xx term
        'u':      r_param,        # u term
        'uuu': -r_param,        # u^3 term
    }
...
class AllenCahn:
    ...
    def simulate(self) -> tuple[np.ndarray, float, np.ndarray]:
        """
        Run the Allen–Cahn simulation and return data shaped for SINDy.
    ...
    def get_target_coefs(self, feature_names: list[str]) -> np.ndarray:
        from .allen_cahn import get_target_coefs
